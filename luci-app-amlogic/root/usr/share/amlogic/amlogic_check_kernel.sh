...
tolog() {
    ...
    wget -q -P "${TMP_CHECK_SERVER_FILE}" "${CONFIG_KERNEL_URL}" >/dev/null 2>&1 && sync
    ...
    if [[ "${CURRENT_KERNEL_V}" == "${SERVER_KERNEL_VERSION}" ]]; then
        ...
        # Download boot file
        wget -q -P "${TMP_CHECK_DIR}/${SERVER_KERNEL_BOOT}" "${SERVER_KERNEL_URL}/${SERVER_KERNEL_VERSION}/${SERVER_KERNEL_BOOT}" >/dev/null 2>&1 && sync
        ...
        # Download dtb file
        wget -q -P "${TMP_CHECK_DIR}/${SERVER_KERNEL_DTB}" "${SERVER_KERNEL_URL}/${SERVER_KERNEL_VERSION}/${SERVER_KERNEL_DTB}" >/dev/null 2>&1 && sync
        ...
        wget -q -P "${TMP_CHECK_DIR}/${SERVER_KERNEL_MODULES}" "${SERVER_KERNEL_URL}/${SERVER_KERNEL_VERSION}/${SERVER_KERNEL_MODULES}" >/dev/null 2>&1 && sync
