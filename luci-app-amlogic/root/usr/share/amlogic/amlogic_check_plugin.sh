...
tolog() {
    ...
    wget -q -P "${TMP_CHECK_SERVER_FILE}" "${CONFIG_PLUGIN_URL}" >/dev/null 2>&1 && sync
    ...
    if [[ "${CURRENT_PLUGIN_V}" == "${SERVER_PLUGIN_VERSION}" ]]; then
        ...
        wget -q -P "${TMP_CHECK_DIR}/${SERVER_PLUGIN_FILE}" "${SERVER_PLUGIN_URL}/${SERVER_PLUGIN_VERSION}/${SERVER_PLUGIN_FILE}" >/dev/null 2>&1 && sync
